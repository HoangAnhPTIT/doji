import { useState } from 'react'
import './App.css'
import CompanyInfo from './components/CompanyInfo'
import ModalImage from './components/ModalImage'
import IconInfo from './components/IconInfo'

function App() {
  const [imgModal, setImgModal] = useState("")
  const [dialog, setdialog] = useState(false)
  const [modalTitle, setModalTitle] = useState("")
  const renderMenuItem = () => {
    const items = [
      {
        className: "active",
        content: "GIỚI THIỆU",
        img: "https://doji.vn/wp-content/uploads/2021/04/cong-chinh.jpg"
      },
      {
        className: "",
        content: "LĨNH VỰC HOẠT ĐỘNG",
        img: "https://doji.vn/wp-content/uploads/2022/12/banner_1440x690-copy-scaled.jpg"
      },
      {
        className: "",
        content: "HỆ THỐNG TẬP ĐOÀN",
        img: "https://doji.vn/wp-content/uploads/2021/04/banner-1.jpg"
      },
      {
        className: "",
        content: "TIN TỨC - SỰ KIỆN",
        img: "https://doji.vn/wp-content/uploads/2021/04/Group-3309.jpg"
      },
      {
        className: "",
        content: "LIÊN HỆ"
      }
    ]

    return items.map((item, key) => {
      return (
        <div
          onClick={() => {
            if (item.img) {
              setModalTitle(item.content)
              setImgModal(item.img)
              setdialog(true)
            }

          }}
          className={'menu-item ' + item.className} key={key}>{item.content}</div>
      )
    })
  }

  const renderIntroMenuItem = () => {
    const items = [
      "TẬP ĐOÀN DOJI",
      "BAN LÃNH ĐẠO",
      "TẦM NHÌN – SỨ MỆNH – GIÁ TRỊ VĂN HÓA",
      "CƠ CẤU TỔ CHỨC",
      "GIẢI THƯỞNG"
    ]

    return items.map((item, key) => {
      return (
        <h2 className='intro-menu-item' key={key}>{item}</h2>
      )
    })
  }

  return (
    <>
      <header>
        <img src="https://doji.vn/wp-content/themes/main/assets/images/home/logo.png" alt="" />
        <div className='menu'>
          {
            renderMenuItem()
          }
        </div>
      </header>
      <div className='banner'>
        <img src="https://doji.vn/wp-content/uploads/2021/04/cong-chinh.jpg" alt="" />
        <div className='intro-menu'>
          <div className='intro-menu-container'>
            {renderIntroMenuItem()}
          </div>
        </div>
      </div>
      <ModalImage title={modalTitle} imgSrc={imgModal} dialog={dialog} closeDialog={() => setdialog(false)} />
      <CompanyInfo
        title={"TẬP ĐOÀN DOJI"}
        img={"https://doji.vn/wp-content/uploads/2023/08/00003_DOJI_AAPlusPhoto_-scaled.jpg"}
        text1="TẬP ĐOÀN VÀNG BẠC ĐÁ QUÝ DOJI, tiền thân là Công ty Phát triển Công nghệ và Thương mại TTD được thành lập ngày 28/07/1994."
        text2="TTD nhanh chóng trở thành Công ty đầu tiên đã đưa ra thị trường quốc tế sản phẩm đá Ruby sao Việt Nam với thương hiệu Việt Nam Star Ruby - VSR. Kể từ đó, Công ty được mệnh danh là ông hoàng Ruby sao của thế giới, đưa Việt Nam được khắc họa như một điểm sáng trên bản đồ Đá quý quốc tế."
        text3="Từ năm 2009 - 2023, Tập đoàn DOJI không ngừng phát triển vượt trội trong những lĩnh vực bản lề về Vàng bạc Đá quý, mở rộng đầu tư Bất động sản, du lịch, nhà hàng, lấn sân sang lĩnh vực Tài chính Ngân hàng và tái cơ cấu thành công Ngân hàng TMCP TPBank." />

      <div className='margin-top-90 history-intro'>
        <img className='history-intro-img' src="https://doji.vn/wp-content/themes/main/assets/images/introduce/intro-decor2.png" alt="" />

      </div>

      <CompanyInfo title="NĂM 2012"
        img={"https://doji.vn/wp-content/uploads/2023/08/mavi1726-3920-1.jpg"}
        isRevertCol={true}
        text1="DOJI tái cơ cấu thành công Ngân hàng TMCP Tiên Phong."
        text2="Với mục tiêu đi đầu về Ngân hàng số, TPBank đã tập trung đầu tư để có hạ tầng hiện đại, giải pháp công nghệ đột phá như LiveBank – ngân hàng tự động 24/7, thanh toán bằng mã QR QuickPay, ngân hàng điện tử Ebank,…"
        text3="Đồng thời, TPBank cũng ứng dụng thành công trí tuệ nhân tạo AI vào hoạt đông trải nghiệm khách hàng. Những sản phẩm vượt trội đó đã giúp TPBank trở thành nhà băng đầu tiên có hệ sinh thái ngân hàng số đa dạng và vượt trội tại Việt Nam."
      />

      <CompanyInfo title="NĂM 2019"
        img={"https://doji.vn/wp-content/uploads/2023/08/HLD1227-scaled.jpg"}
        text1="Tòa nhà DOJI Tower - Trụ sở chính của Tập đoàn chính thức được khai trương, trở thành Trung tâm Vàng bạc Đá quý và Trang sức lớn nhất cả nước với lối kiến trúc đẳng cấp, độc đáo tại số 5 Lê Duẩn, quận Ba Đình, Hà Nội."
        text2="Đây là mốc son chào mừng kỷ niệm 25 năm ngày thành lập Tập đoàn (28/07/1994 - 28/07/2019)."
      />

      <CompanyInfo title="NĂM 2020"
        isRevertCol={true}
        img={"https://doji.vn/wp-content/uploads/2023/08/Screenshot-2023-08-01-135125-1.png"}
        text1="Tập đoàn Vàng bạc Đá quý DOJI chính thức tiếp quản Công ty Thế giới Kim Cương – Top 3 doanh nghiệp Bán lẻ Trang sức lớn nhất Việt Nam."
        text2="Thương vụ diễn ra đúng vào giai đoạn khủng hoảng kinh tế do tác động của đại dịch Covid. Điều này càng khẳng định DOJI luôn tìm thấy thời cơ trong khủng hoảng, tìm thấy cơ hội trong khó khăn, thách thức."
      />

      <CompanyInfo title="NĂM 2022"
        img={"https://doji.vn/wp-content/uploads/2023/08/sap-khai-truong-doji-tower-va-trung-tam-trang-suc-cao-cap-bac-nhat-tai-tp-hcm-05.jpg"}
        text1="DOJI ra mắt DOJI Tower tại số 81 - 85 Hàm Nghi, Quận 1, TP HCM."
        text2="Tòa nhà cao 19 tầng với tổng diện tích mặt sàn lên tới 12.000 m2, được đánh giá là Trung tâm Trang sức cao cấp bậc nhất, mang đậm nét sang trọng của một công trình kiến trúc độc đáo giữa lòng thành phố."
        text3="DOJI Tower TP.HCM là nơi trưng bày những bộ sưu tập trang sức độc đáo, thời thượng và ứng dụng các công nghệ hiện đại hàng đầu thế giới, mang đến cho khách hàng những trải nghiệm mua sắm dẳng cấp và khác biệt."
      />

      <div className="intro-end">
        <div className='intro-end-container'>
          <div className='intro-end-content'>
            <p>Tập đoàn DOJI đã tạo một dấu ấn mạnh mẽ trong cộng đồng doanh nghiệp Việt Nam với tốc độ tăng trưởng doanh thu ấn tượng và giá trị thương hiệu ngày càng lớn mạnh.</p>
            <p>Tập đoàn vinh dự đón nhận Huân chương Lao động Hạng Nhất của Chủ tịch nước CHXHCN Việt Nam trong dịp kỷ niệm 25 năm thành lập (2019), Huân chương Lao động Hạng Nhì trong dịp kỷ niệm 20 năm thành lập (2014) và Huân chương Lao động Hạng Ba (2011).</p>
            <p>Sự vươn lên mạnh mẽ đã đưa DOJI trở thành Doanh nghiệp được Tổ chức đánh giá xếp hạng VNR 500 xếp hạng là Top 5 Doanh nghiệp tư nhân lớn nhất Việt Nam trong 13 năm liên tiếp (2010 - 2022). Ngoài ra, trong bảng tổng hợp các doanh nghiệp thuộc mọi thành phần kinh tế, DOJI thăng hạng liên tục và hiện nay đứng thứ 11 trong số 500 Doanh nghiệp lớn nhất Việt Nam. Cũng trong 12 năm liên tiếp (2012 - 2023), Tập đoàn được công nhận là Thương hiệu Quốc Gia Việt Nam.</p>
            <p>Với tầm nhìn chiến lược, quan điểm phát triển bền vững, tiềm lực hùng mạnh, Tập đoàn DOJI tiếp tục khẳng định đẳng cấp thương hiệu quốc gia Việt Nam, sẵn sàng tâm thế hội nhập và vươn xa trong khu vực cũng như trên trường quốc tế trong tương lai.</p>
          </div>
        </div>
      </div>

      <footer className='footer'>
        <div className="footer-container">
          <div className='footer-container-top'>
            <div className='address-info'>
              <a href="">
                <svg width="240" height="89" viewBox="0 0 240 89" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M127.338 28.6091C121.073 21.718 112.553 21.718 105.913 21.718H89.6248C87.3695 21.718 87.119 22.8456 87.119 23.5974C87.119 23.848 87.2442 25.4768 89.3742 25.4768C90.126 25.4768 90.8777 25.6021 91.1283 25.6021C92.1306 25.8526 92.2559 25.9779 92.3812 27.6067C92.5065 29.862 92.5065 58.9297 92.2559 61.185C92.1306 62.1873 90.2512 63.8161 89.3742 63.8161H89.1236C87.7454 63.8161 86.9937 64.6931 86.9937 65.5702C86.9937 66.5725 87.7454 67.5749 89.3742 67.5749C89.8754 67.5749 93.0077 67.5749 96.7664 67.5749C101.653 67.5749 107.416 67.5749 108.544 67.5749C118.442 67.5749 123.328 63.8161 126.461 60.5585C128.34 58.5538 132.725 52.9157 132.725 43.8947C132.85 36.2519 129.843 31.3655 127.338 28.6091ZM108.419 62.1873C105.036 62.1873 104.535 61.4356 104.033 60.6838V60.5585C103.908 60.3079 103.783 59.5562 103.657 56.925C103.657 56.048 103.783 29.6114 103.783 27.3562C104.284 27.2309 105.412 27.1056 106.414 27.1056C109.922 27.1056 113.054 27.9826 117.189 32.3678C118.693 33.8713 121.198 37.6301 121.073 45.2729C120.948 51.6628 119.319 56.2986 116.187 59.1803C113.43 61.6861 110.173 62.062 108.419 62.1873Z" fill="white"></path>
                  <path d="M181.089 32.869C179.084 29.4861 176.202 26.7297 172.569 24.725C168.935 22.7204 165.177 21.718 161.418 21.718H161.293H161.167H161.042C157.283 21.718 153.525 22.7204 149.891 24.725C146.258 26.7297 143.376 29.4861 141.371 32.869C139.367 36.2519 138.364 40.1359 138.364 44.3959C138.364 48.6558 139.492 52.6651 141.622 56.2986C143.752 59.9321 146.634 62.6885 150.142 64.5679C153.525 66.4472 157.283 67.3243 161.293 67.3243C165.302 67.3243 169.061 66.3219 172.444 64.5679C175.827 62.6885 178.708 59.9321 180.964 56.2986C183.093 52.6651 184.221 48.6558 184.221 44.3959C184.096 40.1359 183.093 36.2519 181.089 32.869ZM161.167 62.062C157.785 62.062 156.156 61.3103 153.775 58.8044C151.395 56.2986 150.267 51.4122 150.267 44.3959C150.267 38.006 151.52 33.3702 153.775 30.6137C156.03 27.8573 157.785 27.1056 161.167 26.9803C164.55 27.1056 166.304 27.8573 168.56 30.6137C170.94 33.3702 172.068 38.006 172.068 44.3959C172.068 51.4122 170.815 56.1733 168.56 58.8044C166.304 61.3103 164.55 62.062 161.167 62.062Z" fill="white"></path>
                  <path d="M207.149 21.8433C206.773 21.718 206.397 21.718 205.896 21.718H189.608V66.0714C189.608 70.9577 188.355 83.8628 182.717 85.4916H182.592C181.715 85.7422 181.213 86.6192 181.339 87.4963C181.464 88.3733 182.216 88.9998 183.093 88.9998H183.218C185.599 88.8745 188.731 86.8698 192.991 82.4846C198.504 76.7212 201.135 67.199 201.135 60.8091C201.135 58.9297 201.135 48.2799 201.135 38.883V28.4838C201.135 26.6044 202.388 26.1032 204.768 25.6021C204.894 25.6021 205.019 25.6021 205.144 25.4768C205.395 25.4768 205.52 25.3515 205.645 25.3515C205.771 25.3515 205.771 25.3515 206.272 25.2262C206.898 25.1009 208.402 24.9756 208.402 23.3468C208.276 22.7204 207.775 22.0939 207.149 21.8433Z" fill="white"></path>
                  <path d="M236.844 63.8161C236.342 63.8161 236.342 63.8161 236.217 63.6908C236.092 63.6908 235.967 63.6908 235.716 63.5655C235.591 63.5655 235.34 63.5655 235.215 63.4402C232.333 63.0644 230.328 62.3126 230.328 60.6838V28.4838C230.328 26.7297 232.208 26.1032 235.215 25.7274C235.465 25.7274 235.591 25.7274 235.716 25.6021C235.967 25.6021 236.217 25.4768 236.217 25.4768C236.342 25.4768 236.468 25.4768 236.844 25.3515C237.595 25.2262 239.224 25.1009 239.224 23.4721C239.224 22.7204 238.723 21.9686 237.971 21.8433C237.595 21.718 237.094 21.718 236.593 21.718H212.412C211.91 21.718 211.409 21.718 211.033 21.8433C210.282 22.0939 209.781 22.7204 209.781 23.4721C209.781 25.1009 211.409 25.2262 212.161 25.3515C212.662 25.3515 212.662 25.3515 212.788 25.4768C212.913 25.4768 213.038 25.4768 213.289 25.6021C213.414 25.6021 213.665 25.6021 213.79 25.7274C216.672 26.1032 218.676 26.855 218.676 28.4838V60.8091C218.676 62.5632 216.797 63.1896 213.79 63.5655C213.539 63.5655 213.414 63.5655 213.289 63.6908C213.038 63.6908 212.913 63.8161 212.788 63.8161C212.537 63.8161 212.537 63.8161 212.161 63.9414C211.409 64.0667 209.781 64.192 209.781 65.8208C209.781 66.5725 210.282 67.3243 211.033 67.4496C211.409 67.5749 211.91 67.5749 212.412 67.5749H236.593C237.094 67.5749 237.595 67.5749 237.971 67.4496C238.723 67.199 239.224 66.5725 239.224 65.8208C239.224 64.0667 237.595 63.9414 236.844 63.8161Z" fill="white"></path>
                  <path d="M64.4407 37.0037C63.9396 35.2496 63.4384 33.4955 62.9372 31.7414C61.3084 26.4792 59.3038 21.3422 56.9232 16.8317C56.1715 15.3282 55.2944 13.8247 54.5427 12.5718C53.1645 10.3165 51.661 8.43712 50.1575 6.93362C46.1481 2.92428 42.5147 1.42077 38.8812 1.67136C37.503 1.67136 36.1248 2.04723 34.6213 2.5484C28.9831 4.67836 23.4703 10.4418 17.2057 16.5811C11.4423 22.3445 6.30529 27.4815 3.67416 32.6185C2.92241 34.2473 2.29595 35.8761 2.17066 37.5048C1.79479 41.3889 3.173 45.2729 7.43292 49.4076C9.68818 51.5375 12.6952 53.6675 16.3286 55.6722C17.7069 56.4239 19.3357 57.1757 20.8392 57.9274C27.6049 60.9344 35.373 63.4403 42.5147 64.8185C44.1435 65.1944 45.647 65.445 47.2758 65.5702C55.1691 66.6979 61.6843 66.322 64.1902 63.6909C68.0742 60.0574 67.4478 48.9064 64.4407 37.0037Z" fill="white"></path>
                  <path d="M54.9195 67.45C52.6643 67.45 50.0332 67.1994 47.2767 66.8236C45.7732 66.573 44.1444 66.3224 42.5157 66.0718C34.9981 64.5683 27.23 62.0625 20.4643 59.0555C18.8355 58.3037 17.332 57.552 15.8285 56.8002C11.9444 54.6702 8.81212 52.5403 6.55687 50.285C2.29694 46.0251 0.417564 41.7652 0.918731 37.2547C1.16932 35.5006 1.67048 33.7465 2.54753 31.8671C5.05337 26.7301 10.3156 21.4679 15.8285 15.955L16.8308 14.9527C23.0954 8.68811 28.483 3.30056 34.2464 1.0453C35.8752 0.418845 37.3787 0.0429688 38.8822 0.0429688C39.1328 0.0429688 39.2581 0.0429688 39.5086 0.0429688C43.518 0.0429688 47.2767 1.92235 51.1608 5.8064C52.6643 7.3099 54.1678 9.31457 55.7966 11.6951C56.6736 13.0733 57.4254 14.4515 58.3024 16.0803C60.5577 20.4656 62.6876 25.7278 64.3164 31.2407C64.8176 32.9947 65.3188 34.8741 65.8199 36.6282C67.0729 41.7652 70.9569 59.0555 65.5694 64.5683C63.5647 66.573 60.0565 67.45 54.9195 67.45Z" fill="white"></path>
                  <path d="M93.8863 27.4818C93.761 25.6024 93.3852 24.7254 91.5058 24.2242C91.1299 24.0989 90.1276 24.0989 89.3758 24.0989C88.7494 24.0989 88.4988 23.9736 88.4988 23.5977C88.4988 23.2219 88.8747 23.0966 89.6264 23.0966C92.5081 23.0966 102.531 23.0966 105.789 23.0966C112.555 23.0966 120.323 23.0966 126.337 29.4864C129.093 32.3682 131.599 37.1293 131.599 43.7697C131.599 50.9114 128.717 56.2989 125.71 59.4312C123.205 62.0623 118.819 66.0717 108.796 66.0717C106.791 66.0717 90.754 66.0717 89.6264 66.0717C88.8747 66.0717 88.4988 65.8211 88.4988 65.4452C88.4988 65.1946 88.7494 64.9441 89.3758 64.9441C91.0046 65.0694 93.5105 62.8141 93.6357 61.1853C93.8863 58.93 93.8863 29.2359 93.8863 27.4818ZM102.281 56.9254C102.281 58.3036 102.406 60.5588 102.907 61.1853C103.659 62.3129 104.286 63.4406 108.545 63.4406C113.181 63.4406 122.077 60.6841 122.453 45.2732C122.578 38.1316 120.448 33.7464 118.193 31.3658C114.058 26.9806 110.675 25.7277 106.416 25.7277C105.288 25.7277 103.534 25.853 103.033 26.1036C102.531 26.3541 102.406 26.6047 102.406 27.2312C102.406 29.1106 102.281 55.923 102.281 56.9254Z" fill="#231F20"></path>
                  <path d="M179.835 33.6211C177.955 30.3635 175.324 27.8577 171.816 25.9783C168.433 24.0989 164.925 23.0966 161.291 23.0966H161.041H160.79C157.157 23.0966 153.649 24.0989 150.266 25.9783C146.883 27.8577 144.126 30.4888 142.247 33.6211C140.368 36.8787 139.365 40.5121 139.365 44.5215C139.365 48.5308 140.368 52.2896 142.498 55.7978C144.502 59.1806 147.259 61.8118 150.391 63.5658C153.649 65.3199 157.157 66.197 160.915 66.197C164.674 66.197 168.182 65.3199 171.44 63.5658C174.698 61.8118 177.329 59.1806 179.333 55.7978C181.338 52.4149 182.466 48.6561 182.466 44.5215C182.716 40.3868 181.839 36.7534 179.835 33.6211ZM169.561 59.6818C166.929 62.4382 164.925 63.4406 161.166 63.4406C157.282 63.4406 155.403 62.4382 152.771 59.6818C150.14 56.8001 148.887 51.7884 148.887 44.3962C148.887 37.6304 150.14 32.744 152.771 29.737C155.277 26.73 157.282 25.7277 161.166 25.7277C165.05 25.853 167.055 26.8553 169.561 29.737C172.192 32.744 173.445 37.6304 173.445 44.3962C173.445 51.7884 172.192 56.9254 169.561 59.6818Z" fill="#231F20"></path>
                  <path d="M205.77 23.0966C204.767 23.0966 190.86 23.0966 190.86 23.0966V66.197C190.86 66.197 190.86 84.6149 182.967 86.8701C182.465 86.9954 182.591 87.7472 183.092 87.7472C185.096 87.6219 187.853 85.7425 191.862 81.6079C197.25 75.9697 199.63 66.6981 199.63 60.8094C199.63 58.053 199.63 36.8787 199.63 28.4841C199.63 24.9759 203.264 24.6001 204.767 24.2242C205.519 24.0989 205.268 24.0989 205.895 24.0989C206.647 23.9736 206.897 23.9736 206.897 23.5977C206.897 23.0966 206.647 23.0966 205.77 23.0966Z" fill="#231F20"></path>
                  <path d="M236.717 65.195C235.965 65.0697 236.216 65.195 235.464 65.0697C233.71 64.8191 228.949 64.4432 228.949 60.8097C228.949 58.1786 228.949 54.921 228.949 51.4129C228.949 47.7794 228.949 42.7677 228.949 38.0066C228.949 34.4984 228.949 31.1156 228.949 28.6097C228.949 25.1015 233.71 24.7257 235.464 24.3498C236.216 24.2245 235.965 24.2245 236.717 24.2245C237.469 24.0992 237.845 24.0992 237.845 23.7233C237.845 23.3475 237.469 23.2222 236.467 23.2222C235.339 23.2222 213.413 23.2222 212.285 23.2222C211.283 23.2222 210.907 23.2222 210.907 23.7233C210.907 24.0992 211.283 24.2245 212.035 24.2245C212.786 24.3498 212.536 24.2245 213.288 24.3498C215.042 24.6004 219.803 24.9763 219.803 28.6097C219.803 31.2409 219.803 34.4984 219.803 38.0066C219.803 42.7677 219.803 47.9047 219.803 51.4129C219.803 54.921 219.803 58.3039 219.803 60.8097C219.803 64.3179 215.042 64.6938 213.288 65.0697C212.536 65.195 212.786 65.195 212.035 65.195C211.283 65.3203 210.907 65.3203 210.907 65.6961C210.907 66.072 211.283 66.1973 212.285 66.1973C213.413 66.1973 235.339 66.1973 236.467 66.1973C237.469 66.1973 237.845 66.1973 237.845 65.6961C237.845 65.3203 237.594 65.195 236.717 65.195Z" fill="#231F20"></path>
                  <path d="M179.835 33.6211C177.955 30.3635 175.324 27.8577 171.816 25.9783C168.433 24.0989 164.925 23.0966 161.291 23.0966H161.041H160.79C157.157 23.0966 153.649 24.0989 150.266 25.9783C146.883 27.8577 144.126 30.4888 142.247 33.6211C140.368 36.8787 139.365 40.5121 139.365 44.5215C139.365 48.5308 140.368 52.2896 142.498 55.7978C144.502 59.1806 147.259 61.8118 150.391 63.5658C153.649 65.3199 157.157 66.197 160.915 66.197C164.674 66.197 168.182 65.3199 171.44 63.5658C174.698 61.8118 177.329 59.1806 179.333 55.7978C181.338 52.4149 182.466 48.6561 182.466 44.5215C182.716 40.3868 181.839 36.7534 179.835 33.6211ZM169.561 59.6818C166.929 62.4382 164.925 63.4406 161.166 63.4406C157.282 63.4406 155.403 62.4382 152.771 59.6818C150.14 56.8001 148.887 51.7884 148.887 44.3962C148.887 37.6304 150.14 32.744 152.771 29.737C155.277 26.73 157.282 25.7277 161.166 25.7277C165.05 25.853 167.055 26.8553 169.561 29.737C172.192 32.744 173.445 37.6304 173.445 44.3962C173.445 51.7884 172.192 56.9254 169.561 59.6818Z" fill="#231F20"></path>
                  <path d="M64.4407 37.0037C63.9396 35.2496 63.4384 33.4955 62.9372 31.7414C61.3084 26.4792 59.3038 21.3422 56.9232 16.8317C56.1715 15.3282 55.2944 13.8247 54.5427 12.5718C53.1645 10.3165 51.661 8.43712 50.1575 6.93362C46.1481 2.92428 42.5147 1.42077 38.8812 1.67136C37.503 1.67136 36.1248 2.04723 34.6213 2.5484C28.9831 4.67836 23.4703 10.4418 17.2057 16.5811C11.4423 22.3445 6.30529 27.4815 3.67416 32.6185C2.92241 34.2473 2.29595 35.8761 2.17066 37.5048C1.79479 41.3889 3.173 45.2729 7.43292 49.4076C9.68818 51.5375 12.6952 53.6675 16.3286 55.6722C17.7069 56.4239 19.3357 57.1757 20.8392 57.9274C27.6049 60.9344 35.373 63.4403 42.5147 64.8185C44.1435 65.1944 45.647 65.445 47.2758 65.5702C55.1691 66.6979 61.6843 66.322 64.1902 63.6909C68.0742 60.0574 67.4478 48.9064 64.4407 37.0037Z" fill="white"></path>
                  <path d="M40.3865 29.3609C38.883 18.711 36.8783 8.5624 34.7484 2.4231C29.1102 4.55306 23.5974 10.3165 17.3328 16.4558C11.5694 22.2192 6.4324 27.3562 3.80127 32.4932C11.4441 30.2379 26.855 29.1103 40.3865 29.3609Z" fill="#DA2031"></path>
                  <path d="M54.5441 12.5718C53.1658 10.3165 51.6623 8.43712 50.1588 6.93362C46.1495 2.92427 42.516 1.42077 38.8826 1.67136C41.2631 6.55774 43.5184 15.9546 45.1472 26.2286C49.2818 21.0916 52.6647 16.2052 54.5441 12.5718Z" fill="#DA2031"></path>
                  <path d="M62.9381 31.7415C61.3093 26.4793 59.3046 21.3423 56.9241 16.8318C55.17 20.34 52.163 24.8505 48.5295 29.6116C54.9194 29.9874 60.1817 30.7392 62.9381 31.7415Z" fill="#DA2031"></path>
                  <path d="M38.1301 34.7484C24.5986 34.6231 9.9394 35.3748 2.42188 37.5048C2.04601 41.3888 3.42422 45.2729 7.68414 49.4075C9.9394 51.5375 12.9464 53.6674 16.5799 55.6721C20.9651 52.9157 29.8608 44.1452 38.1301 34.7484Z" fill="#DA2031"></path>
                  <path d="M64.4426 37.0039C60.8092 36.0016 54.1687 35.2499 46.5259 34.9993C48.28 48.1549 48.9064 60.9347 47.5282 65.5705C55.4216 66.6981 61.9368 66.3222 64.4426 63.6911C68.0761 60.0577 67.4496 48.9067 64.4426 37.0039Z" fill="#DA2031"></path>
                  <path d="M41.5124 38.3823C33.7443 47.5286 25.2245 55.9232 21.0898 57.8026C27.8556 60.8096 35.6237 63.3154 42.7653 64.6936C43.1412 58.9302 42.6401 48.9068 41.5124 38.3823Z" fill="#DA2031"></path>
                </svg>
              </a>

              <p className='footer-text'>
                CÔNG TY CỔ PHẦN <br />
                TẬP ĐOÀN VÀNG BẠC ĐÁ QUÝ DOJI
              </p>
            </div>

            <div className='address-info'>
              <p className='margin-left-30 margin-bottom-unset'>TRỤ SỞ CHÍNH</p>
              <div className='icon-info-list'>
                <IconInfo title={"Toà nhà DOJI Tower, 5 Lê Duẩn,P. Điện Biên, Q.Ba Đình, Hà Nội"}/>
                <IconInfo title={"0288 885 4656"}/>
                <IconInfo title={"0288 885 4656"}/>
                <IconInfo title={"info@doij.com.vn"}/>
              </div>
            </div>


          </div>
        </div>
      </footer>
    </>
  )
}

export default App
